...
class BaseBGCModel(ABC):
    """
    Abstract Base Class for all BGC Models.
    Enforces standard naming for initialization.
    """
    ...
    def initialize(self, restart_file=None, climatology_file=None, **kwargs):
        """
        Universal Initialization.
        Relies on strict naming convention: Model Tracer Name == File Variable Name.
        """
        # 1. RESTART (Exact Match)
        if restart_file:
            print(f"  [Init] Loading Restart: {restart_file}")
            with xr.open_dataset(restart_file) as ds:
                for name in self.names:
                    if name in ds:
                        data = np.nan_to_num(ds[name].values)
                        self.tracers[name][:] = np.where(self.water_mask, data, 0.0)
                    else:
                        print(f"    WARNING: '{name}' missing in restart! Setting to 0.0")
                        self.tracers[name][:] = 0.0
        
        # 2. CLIMATOLOGY (Standard List Check)
        elif climatology_file:
            print(f"  [Init] Initializing from Climatology: {climatology_file}")
            with xr.open_dataset(climatology_file) as ds:
                for name in self.names:
                    # A. If the tracer exists in the file (e.g., 'nitrate'), Load it.
                    if name in ds:
                        print(f"    -> Loading '{name}' from file.")
                        data = np.nan_to_num(ds[name].values)
                        self.tracers[name][:] = np.where(self.water_mask, data, 0.0)
                    
                    # B. If not in file (e.g., 'phyto', 'ammonium'), Seed it.
                    else:
                        seed_val = 0.01  # Small seed for biology to start
                        print(f"    -> '{name}' not in file. Seeding with {seed_val}.")
                        self.tracers[name][:] = np.where(self.water_mask, seed_val, 0.0)

        else:
            print("  [Init] Warning: No file provided. Initializing with Zeros.")
...
@njit(parallel=True, fastmath=True)
def biology_npzd_sasai(no3, phy, zoo, det, temp, par_3d, dz, dt):
    """
    A simple NPZD model base on Sasai et al. 2022
    Tracer Order: no3, phy, zoo, det
    """
    nz, ny, nx = no3.shape
    
    # --- PARAMETERS (start with p_) ---
    p_res = 0.12 # respiration rate (1/d)
    p_pmo = 0.24 # phy mortality rate (1/d)
    p_exc = 0.135 # extracellular excretion rate
    p_gra = 1.4 # maximum grazing rate (1/d)
    p_ivl = 1.4 # ivlev constant (mmol/m3)
    p_p2z = 0.04
    p_aef = 0.7 # assimilation efficiency (-)
    p_gef = 0.3 # growth efficiency (-)
    p_zmo = 0.12 # zooplankton mortality rate (1/d)
    p_dec = 0.3 # decomposition rate (1/d)
    p_gro = 1.5 # maximum growth rate (1/d)
    p_upt = 1 # maximum uptake rate (1/d)
    p_aff = 2 # maximum affinity
    p_ini = 2 # chl-specific initial slope of growth (-)
    p_the = 0.6 # chl-to-carbon ratio
    p_act = 4.8e4 # activation energy
    p_gas = 8.3145 # gas constant
    p_ref = 20 # reference temperature (degC)
    ...
    for j in prange(ny):
        for i in range(nx):
            ...
            for k in range(nz):
                ...
                lim_tem = np.exp(-p_act/p_gas*(1/(TEM+278)-1/(p_ref+278)))
                lim_lig = 1 - np.exp(-p_ini*p_the*PAR/p_gro/lim_tem)
                r_gro = p_gro * NO3 / (NO3 + p_upt/p_aff + 2*np.sqrt(p_upt*NO3/p_aff)) * lim_lig * lim_tem * PHY
                r_pre = p_res * PHY
                r_pmo = p_pmo * PHY * PHY
                r_pex = p_exc * r_gro
                # check with sasai-san but i think Z in A6 is a typo?
                r_gra = p_gra * (1 - np.exp(p_ivl*(p_p2z-PHY))) * ZOO
                r_zmo = p_zmo * ZOO * ZOO
                r_dec = p_dec * DET
                ...
                dNO3 = - r_gro + r_pre + r_pex + (p_aef - p_gef)*r_gra + r_dec
                dPHY = r_gro - r_pre - r_pmo - r_pex - r_gra
                dZOO = r_gra - (p_aef - p_gef)*r_gra - (1 - p_aef)*r_gra - r_zmo
                # sinking is done in anotehr routine???
                dDET = r_pmo + (1 - p_aef)*r_gra + r_zmo - r_dec
                
                # --- F. UPDATE ---
                no3_new[k, j, i] = NO3 + dNO3 * dt_day
                phy_new[k, j, i] = PHY + dPHY * dt_day
                zoo_new[k, j, i] = ZOO + dZOO * dt_day
                det_new[k, j, i] = DET + dDET * dt_day
                
    return no3_new, phy_new, zoo_new, det_new
